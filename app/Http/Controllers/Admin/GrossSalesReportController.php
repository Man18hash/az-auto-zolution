...
class GrossSalesReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        $report = [];
        $grand = ['sales' => 0, 'ar' => 0, 'expenses' => 0, 'deposits' => 0, 'gross' => 0];
        ...
        foreach ($dates as $date) {
            ...
            $depositsForDay = $cashDeposits->filter(fn($dep)=> Carbon::parse($dep->date)->format('Y-m-d') === $date);
    ...
    public function export(Request $request)
    {
        ...
        // ← Here we include the vehicle fields into each row
